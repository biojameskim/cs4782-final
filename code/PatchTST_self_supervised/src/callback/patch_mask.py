...
class PatchMaskCB(Callback):
    def __init__(self, patch_len, stride, mask_ratio=None,
                 mask_when_pred:bool=False):
        """
        Callback used to perform the pretext task of reconstructing the original data after a binary mask has been applied.
        Args:
            patch_len:        patch length
            stride:           stride
            mask_ratio:       mask ratio (if None, use random value between 0.2 and 0.6)
        """
        self.patch_len = patch_len
        self.stride = stride
        self.mask_ratio = mask_ratio        
        self.randomize_mask = mask_ratio is None  # If no mask ratio provided, randomize it
    ...
    def patch_masking(self):
        ...
        # Randomize mask_ratio if needed
        mask_ratio = self.mask_ratio
        if self.randomize_mask:
            mask_ratio = random.uniform(0.2, 0.6)

        xb_mask, _, self.mask, _ = random_masking(xb_patch, mask_ratio)
